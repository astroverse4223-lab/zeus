...
export default function Orb({ size = 120, active = false }) {
  const c = size * 0.56;
  return (
    <>
      <style>{CSS}</style>
      <div style={{ position: 'relative', width: size, height: size, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {/* Outer rings */}
        {[
          { scale: 1.38, a: 'orb-ring-1', d: active ? '1s'   : '3.2s', dl: '0s'   },
          { scale: 1.68, a: 'orb-ring-2', d: active ? '1.3s' : '4.3s', dl: '0.35s'},
          { scale: 2.0,  a: 'orb-ring-3', d: active ? '1.6s' : '5.5s', dl: '0.65s'},
        ].map((r, i) => (
          <div key={i} style={{
            position: 'absolute',
            width: c * r.scale, height: c * r.scale,
            borderRadius: '50%',
            border: `1px solid rgba(0,212,255,${active ? 0.45 : 0.18})`,
        ...
        <div style={{
          ...
          border: `1px dashed rgba(0,212,255,${active ? 0.28 : 0.1})`,
          animation: `orb-spin ${active ? '4s' : '14s'} linear infinite`,
        }} />
        <div style={{
          position: 'absolute', width: c * 0.87, height: c * 0.87, borderRadius: '50%',
          border: `1px dashed rgba(0,212,255,${active ? 0.18 : 0.07})`,
          animation: `orb-spin-rev ${active ? '6s' : '20s'} linear infinite`,
        }} />
        ...
        <div style={{
          ...
          animation: active ? 'orb-pulse-fast 0.75s ease-in-out infinite' : 'orb-pulse 3.8s ease-in-out infinite',
          ...
          {active && (
            <div style={{
              ...
              animation: 'orb-scan 1.4s ease-in-out infinite',
            }} />
          )}
        </div>
      </div>
